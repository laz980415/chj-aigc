...
为了先把前端切到统一入口，当前网关先提供两类入口：

- `/api/**` -> 平台服务
- `/tenant-api/**` -> 租户服务实验入口

也就是说，前端开发环境已经不再直接访问平台服务，而是统一走网关。

为了先把网关服务接起来，仍保留显式租户路由前缀：
...
- 租户服务保留独立实验入口 `/tenant-api/**`
- 下一步再把租户项目、成员、额度接口从平台服务迁到租户服务，并逐步把这部分 `/api/tenant/**` 路由切给租户服务
